...
export function Navbar() {
  ...
  return (
    <header className="w-full sticky top-0 z-50 bg-background/95 backdrop-blur-sm border-b border-foreground/10">
        ...
        <nav className="hidden md:flex items-center space-x-6">
          {navLinks.map((link) => (
            <Link 
              key={link.key} 
              href={link.href}
              className="text-sm font-medium text-foreground/70 hover:text-foreground hover:bg-primary/50 py-1 px-1 rounded-md transition-colors flex items-center space-x-2"
            >
      ...
      <div 
        className={`md:hidden absolute w-full bg-background border-t border-foreground/10 transition-all duration-300 ease-in-out ${
        ...
        <div className="flex flex-col space-y-3 px-6 pb-4">
          {navLinks.map((link) => (
            <Link 
              ...
              href={link.href}
              onClick={toggleMenu}
              className="text-base font-medium text-foreground/80 hover:text-foreground hover:bg-primary/50 py-1 px-1 rounded-md transition-colors flex items-center space-x-3"
            >
