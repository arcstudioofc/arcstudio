...
export default function LocaleSwitcher({ isElectronHeader = false }: LocaleSwitcherProps) {
  ...
  useEffect(() => {
    const id = window.setTimeout(() => {
      setIsElectron(Boolean(window.windowControls));
    }, 0);
  ...
  const isCompactTrigger = isElectronHeader;
  ...
  return (
    ...
    >
      <DropdownTrigger>
        ...
        >
          <div className="flex min-w-0 items-center gap-2">
            <span className="inline-flex size-6 items-center justify-center rounded-full border border-primary/20 bg-primary/10 text-sm leading-none">
              {selectedLocaleFlag}
            </span>
      ...
      <DropdownMenu
        ...
        classNames={{
          base: `${isCompactTrigger ? "min-w-[230px]" : "min-w-[270px]"} rounded-2xl p-1`,
